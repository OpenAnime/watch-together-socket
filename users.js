...
export const addUser = async (id, token, room) => {
    return new Promise(async (resolve) => {
        try {
            ...
            const {
                body
            } = await request('http://127.0.0.1:1902/api/user/' + decoded.id)
            ...
            const checkUserIsInRoom = users.find(user => user.userID === json.id && user.room === room)
            if (checkUserIsInRoom) return resolve({
...
export const getUserById = (id, room) => {
    let user = users.find(user => (room ? user.room == room && user.userID == id : user.userID == id))
    return user
}
